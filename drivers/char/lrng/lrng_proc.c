...
static int lrng_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, lrng_proc_type_show, NULL);
}

static const struct file_operations lrng_proc_type_fops = {
	.owner		= THIS_MODULE,
	.open		= lrng_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
{
	proc_create("lrng_type", 0444, NULL, &lrng_proc_type_fops);
	return 0;
}
